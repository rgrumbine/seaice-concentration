...
// equivalent to get_lut in the original nt2 file
  ...
  fin = fopen("seaice_TBfyark.tab.amsr3","r");
  ...
  fin = fopen("seaice_TBccark.tab.amsr3","r");
  for (loc.i = 0; loc.i < n_atm; loc.i++) {
  for (loc.j = 0; loc.j < n_tb; loc.j++) {
    fscanf(fin, "%f", &tmp);
    tbmcc[loc] = tmp;
  ...
  fclose(fin);
  fin = fopen("seaice_TBowark.tab.amsr3","r");
  for (loc.i = 0; loc.i < n_atm; loc.i++) {
  for (loc.j = 0; loc.j < n_tb; loc.j++) {
    fscanf(fin, "%f", &tmp);
    tbmow[loc] = tmp;
  ...
  fclose(fin);
  fin = fopen("seaice_TBthark.tab.amsr3","r");
  for (loc.i = 0; loc.i < n_atm; loc.i++) {
  for (loc.j = 0; loc.j < n_tb; loc.j++) {
    fscanf(fin, "%f", &tmp);
    tbmthin[loc] = tmp;
...
// equivalent to get_lut in the original nt2 file
  ...
  fin = fopen("seaice_TBfyant.tab.amsr3","r");
  for (loc.i = 0; loc.i < n_atm; loc.i++) {
  for (loc.j = 0; loc.j < n_tb; loc.j++) {
    fscanf(fin, "%f", &tmp);
    tbmfy[loc] = tmp;
  ...
  fin = fopen("seaice_TBccant.tab.amsr3","r");
  for (loc.i = 0; loc.i < n_atm; loc.i++) {
  for (loc.j = 0; loc.j < n_tb; loc.j++) {
    fscanf(fin, "%f", &tmp);
    tbmcc[loc] = tmp;
  ...
  fclose(fin);
  fin = fopen("seaice_TBowant.tab.amsr3","r");
  for (loc.i = 0; loc.i < n_atm; loc.i++) {
  for (loc.j = 0; loc.j < n_tb; loc.j++) {
    fscanf(fin, "%f", &tmp);
    tbmow[loc] = tmp;
  ...
  fclose(fin);
  fin = fopen("seaice_TBthant.tab.amsr3","r");
  for (loc.i = 0; loc.i < n_atm; loc.i++) {
  for (loc.j = 0; loc.j < n_tb; loc.j++) {
    fscanf(fin, "%f", &tmp);
    tbmthin[loc] = tmp;
...
//    float x = 999, y=999;
	...
	v11i = (double) v11;
        ...
        v89i=(double) v89;
        h89i=(double) h89;
        amsr3_regress_to_amsre(v19i, h19i, v24i, v37i, h37i, v89i, h89i, lat);
