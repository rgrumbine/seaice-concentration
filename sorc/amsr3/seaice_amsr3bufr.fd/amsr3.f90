...
!F90 and other modifications R. Grumbine 2016
!Adapted for AMSR3 5 August 2026
      
  INTEGER :: amsr3chans 
  PARAMETER (amsr3chans = 19)
  ...
  REAL(8) :: HDR(9), XLOC(2,1)
  REAL(8) :: PRDLR(5,amsr3chans)
...
!
  ...
  LOGICAL ok, lfok, weatherok
!.................................

       OPEN  ( UNIT = iunt, FILE = FLT )

       CALL W3TAGB('DCODAMSR3',0095,0333,0077,'NP11   ')
...
!  -----------------------------------------------
      ...
      CALL UFBrep(LUBFI,PRDLR,5,amsr3chans,iret, HS3)
!      PRINT *,'iret hs3 = ',iret
      nchan = iret
!
      isad   = NINT(hdr(1))
...
! ice.  31 July 2017 Robert Grumbine
          ok = .TRUE.
          IF (nchan .EQ. 2) THEN
            PRINT *,'nchan = 2, should not be here'
            STOP
          ELSE
            ok = lfok(PRDLR)
          ENDIF 
...
!---------------------------------------------------------------
  ...
  IMPLICIT none
  REAL(8) obs(5,19) ! i = parameters, 5 = tmbr; j = channel
  LOGICAL tmp
  INTEGER i
  tmp = .TRUE.
