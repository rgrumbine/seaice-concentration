#!/bin/bash 

export tag=${tag:-`date +"%Y%m%d"`}
export NRT=${NRT:-YES}
echo zzz tag, NRT = $tag, $NRT

tagm=`expr $tag - 1`
export tagm=`/u/robert.grumbine/bin/dtgfix3 $tagm`
tagm2=`expr $tagm - 1`
export tagm2=`/u/robert.grumbine/bin/dtgfix3 $tagm2`
export end=$tag
echo initial tag date = $tag

#-----------------------------------------------------------------------------

set -x

# Bring the various environment-sensitive definitions out of J jobs and to here:
#NCO refers to these as 'job card' variables

export HOMEbase=${HOMEbase:-$HOME/rgops}
export seaice_analysis_ver=${seaice_analysis_ver:-v4.5.2}
export HOMEseaice_analysis=$HOMEbase/seaice_analysis.${seaice_analysis_ver}

#Use this to override system in favor of my archive:
if [ $NRT == 'NO' ] ; then
  export DCOMROOT=${DCOMROOT:-/u/robert.grumbine/noscrub/satellites/prod/}
  export RGTAG=prod
  export my_archive=true
fi

cd $HOMEseaice_analysis/ecf/
source $HOMEseaice_analysis/versions/run.ver
set +x
. ./jobcards
#module load prod_envir/$prod_envir_ver
#module load intel/$intel_ver
module load craype/$craype_ver
module load cray-mpich/${cray_mpich_ver}
module load cray-pals/${cray_pals_ver}
module load cfp/${cfp_ver}
#module load ve/seaice-analysis/${ve_seaice_analysis_ver}
set -xe
echo zzz module list
module list
echo zzz date pdy= $PDY cyc = $cyc ncepdate = $ncepdate

export cyc=${cyc:-00}
export envir=${envir:-developer}
export job=${job:-seaice_analysis}

#--------------------------------------------------------------------------------------
#The actual running of stuff
export KEEPDATA=${KEEPDATA:-NO}

while [ $tag -le $end ]
do
  export PDY=$tag
  export PDYm1=$tagm
  export PDYm2=$tagm2
  echo dates: tag= $tag PDY = $PDY

  export DATA=$DATAROOT/${job}.${pid}

#Now call J job, which will call the ex
  echo zzz calling JSEAICE_amsr3l2
  time $HOMEseaice_analysis/jobs/JSEAICE_amsr3l2

  exit

  tagm2=$tagm
  tagm=$tag
  tag=`expr $tag + 1`
  tag=`/u/robert.grumbine/bin/dtgfix3 $tag`

done
